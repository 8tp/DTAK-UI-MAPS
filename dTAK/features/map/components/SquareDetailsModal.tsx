...
export const SquareDetailsModal: React.FC<Props> = ({
  visible,
  mode,
  square,
  onCancel,
          ...
          <View style={styles.card}>
            <ScrollView contentContainerStyle={{ gap: 12 }}>
              {isCreate ? (
                ...
                <>
                  {square?.title ? (
                    <Text style={styles.viewTitle}>{square.title}</Text>
                  ) : null}
                  {square?.description ? (
                    <Text style={styles.viewDescription}>{square.description}</Text>
                  ) : null}
                  {square?.createdAt ? (
                    <Text style={styles.timestamp}>
                      Created: {new Date(square.createdAt).toLocaleString()}
                    </Text>
                  ) : null}
                </>
              )}
