...
export const CircleDetailsModal: React.FC<Props> = ({
  visible,
  mode,
  circle,
  onCancel,
          ...
          <View style={styles.card}>
            <ScrollView contentContainerStyle={{ gap: 12 }}>
              {isCreate ? (
                ...
                <>
                  {circle?.title ? (
                    <Text style={styles.viewTitle}>{circle.title}</Text>
                  ) : null}
                  {circle?.description ? (
                    <Text style={styles.viewDescription}>{circle.description}</Text>
                  ) : null}
                  {circle?.createdAt ? (
                    <Text style={styles.timestamp}>
                      Created: {new Date(circle.createdAt).toLocaleString()}
                    </Text>
                  ) : null}
                </>
              )}
