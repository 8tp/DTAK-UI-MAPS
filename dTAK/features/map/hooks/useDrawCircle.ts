...
import { approximateCircle, metersBetween, type LonLat } from "../utils/geometry/circle";

type InteractionMode = "DEFAULT" | "DRAW_CIRCLE";

export type CircleFeature = GeoJSON.Feature<GeoJSON.Polygon, {
    id: string;
    center: LonLat;
    radius: number;
    units: "meters";
...
export function useDrawCircle() {
    ...
    const [circles, setCircles] = useState<GeoJSON.FeatureCollection<GeoJSON.Polygon>>({ type: "FeatureCollection", features: [] });
    ...
    const onRelease = useCallback(async (point?: [number, number]) => {
        ...
        setCircles(prev => ({ type: "FeatureCollection", features: [...prev.features, feature] }));
        setPreview(undefined);
        setCenter(undefined);
        setRadius(undefined);
        setMode("DEFAULT");
    ...
    const findCircleAtCoordinate = useCallback((coord: LonLat) => {
